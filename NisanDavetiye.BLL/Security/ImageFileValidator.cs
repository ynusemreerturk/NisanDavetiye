...
{
    public static bool TryDetect(string contentType, ReadOnlySpan<byte> header, out string detectedType)
    {
    ...
    public static async Task<(bool Ok, string DetectedType)> TryDetectAsync(
        string contentType,
        Stream stream,
        CancellationToken cancellationToken = default)
    {
        ...
        var ok = TryDetect(contentType, header.AsSpan(0, read), out var detected);
        return (ok, detected);
    }
